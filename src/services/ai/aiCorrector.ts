...
async function invokeCorrection(body: Record<string, unknown>) {
  const invoke = () => supabase.functions.invoke<FunctionResponse>("ai-exercise-corrector-v3", { body });
  let lastError: unknown = null;
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      const request = invoke();
      const timeout = new Promise<never>((_, reject) =>
        window.setTimeout(() => reject(new Error("Le correcteur IA a dépassé 50 secondes. La requête a été interrompue, tu peux réessayer.")), 50_000),
      );
      ...
      if (!data?.success || !data.data) {
        throw new Error(
          data?.detail
            ? `${data.error ?? "Le correcteur IA est indisponible."} ${data.detail}`
            : data?.error ?? "Le correcteur IA est indisponible.",
        );
      }
      return data.data;
    } catch (error) {
