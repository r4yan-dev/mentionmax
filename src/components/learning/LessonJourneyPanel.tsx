...
const actionLabels = { exercise: "Exercices ciblés", quiz: "Quiz de maîtrise", revision: "Révision ciblée" } as const;
  ...
  async function load() {
    ...
    try {
      setJourney(await getCourseJourney({
        ...
        objectives: lesson.blocks
          .filter((block) => block.type === "concept" || block.type === "method" || block.type === "recap")
          .flatMap((block) => block.items ?? (block.title ? [block.title] : []))
          .slice(0, 6),
      }));
    } catch (err) {
      setError(err instanceof Error ? err.message : "Impossible de construire ton parcours.");
    } finally {
      setLoading(false);
    }
  }

  useEffect(() => { void load(); }, [lesson.id, subjectId, trackId, user?.id]);

  if (!user) return null;
  if (loading && !journey) {
    return <section className="lesson-journey"><div className="lesson-journey__loading"><LoaderCircle size={18} className="spin" /><span>Analyse de ton historique pour préparer la suite…</span></div></section>;
  }
  if (error && !journey) {
    return <section className="lesson-journey"><div className="lesson-journey__error"><span>{error}</span><button type="button" onClick={() => void load()}><RefreshCw size={14} /> Réessayer</button></div></section>;
  }
  if (!journey) return null;

  const actionPath = journey.nextAction.kind === "quiz"
    ? `/exercices?subject=${subjectId}&mode=quiz`
    : `/exercices?subject=${subjectId}&chapter=${encodeURIComponent(lesson.chapter)}`;

  return <section className="lesson-journey">
    <div className="lesson-journey__header">
      <div><span className="lessons-eyebrow">TON PARCOURS ADAPTATIF</span><h2>Comprendre → appliquer → vérifier</h2><p>La suite est choisie à partir de ce que tu viens de voir et de ce que tu rates habituellement.</p></div>
      <button type="button" className="lesson-journey__refresh" onClick={() => void load()} disabled={loading}><RefreshCw size={14} /></button>
    </div>
