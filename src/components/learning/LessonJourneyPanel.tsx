...
import AdaptiveExercisePanel from "./AdaptiveExercisePanel";
...
export default function LessonJourneyPanel({ lesson, subjectId, trackId }: Props) {
  ...
  async function load(options?: { force?: boolean }) {
    ...
    try {
      setJourney(await getCourseJourney({ trackId, subjectId, lessonId: lesson.id, courseTitle: lesson.title, chapter: lesson.chapter, topic: lesson.topic, intro: lesson.blocks.find((block) => block.type === "intro")?.text, objectives: lesson.blocks.filter((block) => block.type === "concept" || block.type === "method" || block.type === "recap").flatMap((block) => block.items ?? (block.title ? [block.title] : [])).slice(0, 6) }, options));
  ...
  if (error && !journey) return <section className="lesson-journey"><div className="lesson-journey__error"><span>{error}</span><button type="button" onClick={() => void load({ force: true })}><RefreshCw size={14} /> Réessayer</button></div></section>;
  ...
  const shared = { lessonId: lesson.id, courseTitle: lesson.title, chapter: lesson.chapter, topic: lesson.topic, intro: lesson.blocks.find((block) => block.type === "intro")?.text, objectives, subjectId, trackId, nextAction: journey.nextAction, onFinished: () => { setPracticeOpen(false); void load({ force: true }); } };
  ...
  return <section className="lesson-journey">
    ...
    {practiceOpen && (journey.nextAction.kind === "exercise" ? <AdaptiveExercisePanel {...shared} /> : <AdaptiveQuizPanel {...shared} />)}
