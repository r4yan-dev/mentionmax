...
const SUPERS: Record<string, string> = { "⁰":"0","¹":"1","²":"2","³":"3","⁴":"4","⁵":"5","⁶":"6","⁷":"7","⁸":"8","⁹":"9","⁺":"+","⁻":"-","ⁿ":"n","ᵃ":"a","ᵇ":"b","ᶜ":"c","ᵈ":"d","ᵉ":"e","ᶠ":"f","ᵍ":"g","ʰ":"h","ⁱ":"i","ʲ":"j","ᵏ":"k","ˡ":"l","ᵐ":"m","ᵒ":"o","ᵖ":"p","ʳ":"r","ˢ":"s","ᵗ":"t","ᵘ":"u","ᵛ":"v","ʷ":"w","ˣ":"x","ʸ":"y","ᶻ":"z" };
...
function removeProseMathDelimiters(value: string) {
  return value.replace(/\\\(([^()\n]+)\\\)/g, (match, body: string) => {
    const text = body.trim();
    const words = text.match(/[A-Za-zÀ-ÿ]{2,}/g) ?? [];
    const prose = words.length >= 2 && /\s/.test(text) && !/[=<>+*/^_{}]/.test(text) && !/\\[A-Za-z]+/.test(text);
    return prose ? text : match;
  });
}
function toLatex(text: string) {
  ...
  return removeProseMathDelimiters(restoreLatex(source,protectedValue.tokens));
}

export function LatexText({ children, className, display=false }: { children: string; className?: string; display?: boolean }) {
  const ref = useRef<HTMLSpanElement>(null);
  const latex = useMemo(() => display ? `\\[${sanitizeProtectedMath(children.trim()).replace(/^\\\[|\\\]$/g, "")}\\]` : toLatex(children), [children,display]);
  useEffect(() => {
    let cancelled=false;
