...
import { BarChart3, BookOpen, BrainCircuit, ChevronDown, FileText, Home, Library as LibraryIcon, LogOut, Menu, Search, Sparkles, Trophy, X } from "lucide-react";
...
const navItems = [
  ...
  { to: "/bibliotheque", label: "Ma bibliothèque", icon: LibraryIcon },
...
const searchTargets: SearchTarget[] = [
  ...
  ["Ma bibliothèque", "Retrouver tes ressources sauvegardées", "/bibliotheque", LibraryIcon],
...
const pageNames: Record<string, string> = { "/accueil": "Accueil", "/subjects": "Matières", "/matieres": "Matières", "/exercices": "Pratique", "/exercises": "Pratique", "/focus": "Focus", "/examens": "Examens", "/exams": "Examens", "/ai-studio": "AI Studio", "/bibliotheque": "Ma bibliothèque", "/ai-help": "Tuteur IA", "/progression": "Progression", "/classement": "Classement", "/profil": "Profil", "/preferences": "Préférences" };
...
export default function AuthenticatedShell() {
  ...
  const pageTitle = pageNames[location.pathname] ?? (location.pathname.startsWith("/exercices/") ? "Pratique" : location.pathname.startsWith("/subjects/") ? "Matières" : location.pathname.startsWith("/exams/") ? "Examens" : location.pathname.startsWith("/focus") ? "Focus" : location.pathname.startsWith("/bibliotheque") ? "Ma bibliothèque" : "MentionMax");
  ...
  return <div className="app-shell">
    ...
    <aside className={`shell-sidebar ${mobileOpen ? "is-open" : ""}`}>
      ...
      <nav className="shell-nav" aria-label="Navigation principale">{navItems.map(({to,label,icon:Icon})=><NavLink key={to} to={to} onClick={()=>setMobileOpen(false)} className={({isActive})=>`shell-nav-link ${isActive || (to === "/exercices" && location.pathname.startsWith("/exercices")) || (to === "/focus" && location.pathname.startsWith("/focus")) || (to === "/examens" && (location.pathname.startsWith("/examens") || location.pathname.startsWith("/exams"))) || (to === "/bibliotheque" && location.pathname.startsWith("/bibliotheque")) ? "active" : ""}`}><Icon size={18} strokeWidth={2}/><span>{label}</span></NavLink>)}</nav>
