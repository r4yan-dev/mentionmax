    ...
    <div className="app-shell">
      ...
      <aside className={`shell-sidebar ${mobileOpen ? "is-open" : ""}`}>
        ...
        <div className="shell-sidebar-bottom">
          <NavLink to="/ai-help" className="shell-tutor-card" onClick={() => setMobileOpen(false)}><span className="shell-tutor-icon"><Sparkles size={15} /></span><span><strong>Besoin d’aide ?</strong><small>Ouvrir le tuteur IA</small></span><ChevronDown size={15} /></NavLink>
          ...
          <button className="shell-profile" onClick={() => navigate("/profil")}>
            ...
            <span><strong>{profile?.display_name || "Mon profil"}</strong><small>Compte étudiant · {pageTitle}</small></span>
            <ChevronDown size={15} />
          </button>
      ...
      <div className="shell-main">
        <header className="shell-topbar">
          ...
          <button className="shell-search" type="button" onClick={openSearch}><Search size={17} /><span>Rechercher un chapitre, exercice...</span><kbd>⌘K</kbd></button>
          <div className="shell-topbar-right"><div className="shell-page-title">{pageTitle}</div><div className="shell-path-chip">{trackLabel}</div><NavLink to="/profil" className="shell-top-profile"><span className="shell-avatar">{initials}</span><span>{profile?.display_name || "Mon profil"}</span></NavLink></div>
        </header>
        <div className="shell-content"><Outlet /></div>
      </div>
      {searchOpen && <div className="shell-command" role="dialog" aria-modal="true" aria-label="Recherche MentionMax">
        <div className="shell-command__panel">
          <div className="shell-command__top"><Search size={18} /><input ref={searchRef} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Rechercher dans MentionMax..." /><kbd>ESC</kbd><button type="button" onClick={() => setSearchOpen(false)} aria-label="Fermer"><X size={17} /></button></div>
          <div className="shell-command__results">
            {searchResults.length ? searchResults.map(([title, description, to, Icon]) => <button type="button" className="shell-result" key={to} onClick={() => go(to)}><span className="shell-result__icon"><Icon size={16} /></span><span><strong>{title}</strong><small>{description}</small></span><span className="shell-result__shortcut">↵</span></button>) : <div className="shell-command__empty">Aucun espace ne correspond à “{query}”.</div>}
          </div>
          <div className="shell-command__footer"><span>⌘K pour ouvrir</span><span>ESC pour fermer</span></div>
        </div>
      </div>}
    </div>
  );
