...
import { BarChart3, BookOpen, BrainCircuit, Bot, ChevronDown, FileText, Home, LogOut, Menu, Search, Sparkles, Trophy, X, type LucideIcon } from "lucide-react";
...
const navItems = [
  ...
  { to: "/ai-help", label: "AI Tutor", icon: Bot },
...
const searchTargets: SearchTarget[] = [
  ...
  ["Ma session", "Construire ou reprendre ta séance dans Focus", "/focus", Sparkles],
  ["Focus", "Travailler seul ou avec ton groupe", "/focus", Sparkles],
  ["Tuteur IA", "Poser une question à MentionMax AI", "/ai-help", Bot],
...
const pageNames: Record<string, string> = { "/accueil": "Accueil", "/subjects": "Matières", "/matieres": "Matières", "/exercices": "Pratique", "/exercises": "Pratique", "/session": "Focus", "/study-agent": "Focus", "/focus": "Focus", "/communaute": "Focus", "/community": "Focus", "/examens": "Examens", "/exams": "Examens", "/ai-studio": "AI Studio", "/bibliotheque": "Ma bibliothèque", "/ai-help": "AI Tutor", "/progression": "Progression", "/points-faibles": "Progression", "/weak-points": "Progression", "/classement": "Classement", "/profil": "Profil", "/preferences": "Préférences" };
...
export default function AuthenticatedShell() {
  ...
  return <div className="app-shell">
    ...
    <aside className={`shell-sidebar ${mobileOpen ? "is-open" : ""}`}>
      ...
      <nav className="shell-nav" aria-label="Navigation principale">{navItems.map(({to,label,icon:Icon})=><NavLink key={to} to={to} onClick={()=>setMobileOpen(false)} className={({isActive})=>`shell-nav-link ${isActive || (to === "/exercices" && (location.pathname.startsWith("/exercices") || location.pathname.startsWith("/exercises"))) || (to === "/focus" && (location.pathname.startsWith("/focus") || location.pathname.startsWith("/communaute") || location.pathname.startsWith("/community") || location.pathname.startsWith("/session") || location.pathname.startsWith("/study-agent"))) || (to === "/examens" && (location.pathname.startsWith("/examens") || location.pathname.startsWith("/exams"))) ? "active" : ""}`}> <Icon size={18} strokeWidth={2}/><span>{label}</span></NavLink>)}</nav>
      <div className="shell-sidebar-bottom"><NavLink to="/ai-help" className="shell-tutor-card" onClick={()=>setMobileOpen(false)}><span className="shell-tutor-icon"><Bot size={15}/></span><span><strong>Tuteur IA</strong><small>Poser une question à MentionMax</small></span><ChevronDown size={15}/></NavLink><button className="shell-streak" onClick={()=>go("/progression")}><span>🔥</span><div><strong>5 jours</strong><small>Série actuelle</small></div></button><button className="shell-profile" onClick={()=>go("/profil")}><span className="shell-avatar">{initials}</span><span><strong>{profile?.display_name || "Mon profil"}</strong><small>Compte étudiant · {pageTitle}</small></span><ChevronDown size={15}/></button><button className="shell-logout" onClick={handleSignOut}><LogOut size={16}/> Déconnexion</button></div>
    </aside>
