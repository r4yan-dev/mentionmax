import { ArrowRight, BrainCircuit, CheckCircle2, Clock3, Target, TrendingUp } from "lucide-react";
  ...
  return <main className="section container">
    ...
    <div className="dashboard-grid">
      <section className="card">
        <div className="section-row"><div><span className="section-eyebrow">Cette semaine</span><h2>Régularité</h2></div><span style={{fontSize:12,color:"#0fa3a3",fontWeight:800}}>+18% vs semaine passée</span></div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",alignItems:"end",height:190,gap:10,paddingTop:15}}>{weekly.map((value,index)=><div key={index} style={{height:`${Math.max(25,value)}%`,borderRadius:"8px 8px 3px 3px",background:index===4?"#0fa3a3":"#d8eeea",position:"relative"}}><span style={{position:"absolute",top:-19,left:"50%",transform:"translateX(-50%)",fontSize:10,fontWeight:750,color:index===4?"#0fa3a3":"#7b9691"}}>{value}%</span></div>)}</div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:10,marginTop:9}}>{["L","M","M","J","V","S","D"].map((day,index)=><span key={index} style={{textAlign:"center",fontSize:10,color:"#91a6a2"}}>{day}</span>)}</div>
      </section>
      <section className="card">
    ...
    <section className="card" style={{marginTop:20,padding:22}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(230px,1fr))",gap:10}}>
        <Link to="/exercices?subject=maths" className="card" style={{padding:16,textDecoration:"none"}}><BrainCircuit size={18}/><h3 style={{margin:"10px 0 5px",fontSize:15}}>Faire 5 exercices</h3><p style={{margin:0,color:"#718582",fontSize:12}}>Consolide ton chapitre le moins stable.</p></Link>
        ...
        <div className="card" style={{padding:16}}><Clock3 size={18}/><h3 style={{margin:"10px 0 5px",fontSize:15}}>25 min aujourd'hui</h3><p style={{margin:0,color:"#718582",fontSize:12}}>Une session courte reste meilleure que zéro.</p></div>
        <div className="card" style={{padding:16}}><CheckCircle2 size={18}/><h3 style={{margin:"10px 0 5px",fontSize:15}}>Objectif hebdo</h3><p style={{margin:0,color:"#718582",fontSize:12}}>72% du volume prévu est déjà réalisé.</p></div>
      </div>
    </section>
  </main>;
}
