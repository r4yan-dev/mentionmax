...
export default function Progression() {
  ...
  return <main className="section container">
    ...
    <div className="dashboard-grid">
      <section className="card"><div className="section-row"><div><span className="section-eyebrow">Cette semaine</span><h2>Régularité</h2></div><span style={{fontSize:12,color:"#0fa3a3",fontWeight:800}}>{objectivePercent}% de l'objectif</span></div><div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",alignItems:"end",height:190,gap:10,paddingTop:15}}>{weekly.map((day)=><div key={day.date} style={{height:`${Math.max(8, (day.focus_minutes / maxFocus) * 100)}%`,borderRadius:"8px 8px 3px 3px",background:day.focus_minutes>0?"#0fa3a3":"#d8eeea",position:"relative",minHeight:8}}><span style={{position:"absolute",top:-19,left:"50%",transform:"translateX(-50%)",fontSize:10,fontWeight:750,color:day.focus_minutes>0?"#0fa3a3":"#7b9691",whiteSpace:"nowrap"}}>{day.focus_minutes} min</span></div>)}</div><div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:10,marginTop:9}}>{weekly.map((day)=><span key={day.date} style={{textAlign:"center",fontSize:10,color:"#91a6a2"}}>{weekdayLabel(day.date)}</span>)}</div></section>
      <section className="card"><div className="section-row"><div><span className="section-eyebrow">Matières</span><h2>Maîtrise</h2></div><Link to="/matieres" className="text-brand">Explorer <ArrowRight size={14}/></Link></div><div style={{display:"flex",flexDirection:"column",gap:17}}>{subjects.length > 0 ? subjects.map((subject)=><div key={subject.slug}><div style={{display:"flex",justifyContent:"space-between",marginBottom:7}}><strong style={{fontSize:12}}>{subject.name}</strong><span style={{fontSize:12,fontWeight:850}}>{Math.round(Number(subject.mastery ?? 0))}%</span></div><ProgressBar value={Math.round(Number(subject.mastery ?? 0))}/></div>) : <div style={{padding:"18px 0",color:"#718582",fontSize:12}}>Aucune donnée de maîtrise enregistrée pour ton compte.</div>}</div></section>
    </div>
