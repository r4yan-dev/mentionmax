...
function TextList({ items }: { items: unknown }) {
  ...
  return <ul className="generator-lesson-list">{items.map((item, index) => <li key={index}>{stringifyValue(item)}</li>)}</ul>;
}
...
function LessonResultView({ result }: { result: ResultRecord }) {
  ...
  return <div className="generator-result generator-lesson">
    <div className="generator-result__head"><div><span className="generator-eyebrow"><Check size={14} /> LEÇON MENTI · TERMINÉE</span><h2>{title}</h2></div></div>
    {typeof result.intro === "string" && <section className="generator-lesson__intro"><span className="generator-result__label">Introduction</span><p>{result.intro}</p></section>}
    ...
    <div className="generator-lesson__sections">
      {sections.map((section, index) => <article className="generator-lesson__section" key={`${String(section.title)}-${index}`}>
        ...
        <div className="generator-lesson__section-body">
          <h3>{typeof section.title === "string" ? section.title : `Partie ${index + 1}`}</h3>
          {typeof section.explanation === "string" && <p className="generator-lesson__explanation">{section.explanation}</p>}
          {typeof section.intuition === "string" && section.intuition.trim() && <div className="generator-lesson__callout"><strong>💡 Intuition</strong><p>{section.intuition}</p></div>}
          <div className="generator-lesson__columns">
            ...
            {Array.isArray(section.definitions) && section.definitions.length > 0 && <div><span className="generator-result__label">Définitions</span>{(section.definitions as ResultRecord[]).map((item, i) => <div className="generator-lesson__definition" key={i}><strong>{stringifyValue(item.term)}</strong><p>{stringifyValue(item.definition)}</p></div>)}</div>}
          </div>
          ...
          {checkpoints(section).length > 0 && <div className="generator-lesson__checkpoints"><span className="generator-result__label">✎ Checkpoints</span>{checkpoints(section).map((item, i) => <details key={i}><summary>{stringifyValue(item.question)}</summary><p><strong>Réponse :</strong> {stringifyValue(item.answer)}</p>{typeof item.why === "string" && item.why.trim() && <p>{item.why}</p>}</details>)}</div>}
    ...
    <div className="generator-lesson__final-grid">
      ...
      <section><span className="generator-result__label">Auto-évaluation</span>{Array.isArray(result.selfAssessment) && (result.selfAssessment as ResultRecord[]).map((item, i) => <details key={i}><summary>{stringifyValue(item.question)}</summary><p>{stringifyValue(item.answer)}</p></details>)}</section>
    </div>
    {Array.isArray(result.progressiveExercises) && result.progressiveExercises.length > 0 && <section className="generator-lesson__exercises"><span className="generator-result__label">Exercices progressifs</span>{(result.progressiveExercises as ResultRecord[]).map((item, i) => <article key={i}><span>{stringifyValue(item.difficulty)}</span><p>{stringifyValue(item.statement)}</p><details><summary>Correction / indication</summary><p>{stringifyValue(item.hint)}</p><p>{stringifyValue(item.answer)}</p></details></article>)}</section>}
...
function ResultView({ result }: { result: ResultRecord }) {
  ...
  return <div className="generator-result"><div className="generator-result__head"><div><span className="generator-eyebrow"><Check size={14} /> GÉNÉRATION TERMINÉE</span><h2>{title}</h2></div></div><div className="generator-result__grid">{entries.map(([key, value]) => <section key={key} className="generator-result__block"><span className="generator-result__label">{key.replace(/([A-Z])/g, " $1").replace(/^./, (char) => char.toUpperCase())}</span><div className="generator-result__content">{Array.isArray(value) ? value.map((item, index) => <article className="generator-result__item" key={`${key}-${index}`}>{item && typeof item === "object" ? Object.entries(item as Record<string, unknown>).map(([itemKey, itemValue]) => <div key={itemKey}><strong>{itemKey.replace(/([A-Z])/g, " $1")}</strong><p>{stringifyValue(itemValue)}</p></div>) : <p>{stringifyValue(item)}</p>}</article>) : <p>{stringifyValue(value)}</p>}</div></section>)}</div></div>;
}
