...
const typeLabels: Record<ExerciseType, string> = { mcq: "QCM", numeric: "Numérique", "short-answer": "Réponse courte", proof: "Démonstration", calculation: "Calcul", "document-analysis": "Analyse de document", "multi-step": "Problème" };
...
function StatementContent({ statement }: { statement: string }) {
  const blocks = statement.split(/\n{2,}/).map((block) => block.trim()).filter(Boolean);
  return (
    <div className="exercise-document-copy">
      {blocks.map((block, index) => (
        <p key={`${block}-${index}`} className={index === 0 ? "exercise-document-lead" : undefined}>
          <LatexText>{block}</LatexText>
        </p>
      ))}
    </div>
  );
    ...
    <main className="exercises-page exercises-detail-page">
      ...
      <article className="exercise-detail-card exercise-detail-card--document">
        <header className="exercise-document-header">
          <div className="exercise-document-header__brand">
            <span className="exercise-document-kicker">MENTIONMAX · 2BAC</span>
            <span className="exercise-document-mode">ENTRAÎNEMENT</span>
          </div>
          <div className="exercise-document-header__rule" />
          <div className="exercise-document-meta">
            <span>{subjectLabels[exercise.target.subjectId]}</span>
            <span>{tracks[exercise.target.trackIds[0]].label}</span>
            {number !== null && <span>Exercice {number}</span>}
          </div>
        </header>

        <section className="exercise-document-title">
          <div className="exercise-document-title__line">
            <span className="exercise-document-section">SUJET</span>
            <div className="exercise-document-badges">
              <span className="exercise-pill">{exerciseTypeLabel(exercise.type)}</span>
          ...
          {exercise.examTip && <div className="exercise-goal exercise-goal--detail"><Target size={17} /><div><strong>Compétence visée</strong><span>{exercise.examTip}</span></div></div>}
        </section>

        <section className="exercise-document-section-block">
          <div className="exercise-document-section-heading">
            <span className="exercise-document-section-number">1</span>
            <div>
              <span className="section-eyebrow">ÉNONCÉ</span>
              <h2>Travail à effectuer</h2>
            </div>
          </div>
          <StatementContent statement={exercise.statement} />
          <ExerciseVisual visual={exercise.visual} />
        </section>

        <section className="exercise-document-section-block exercise-document-response">
          <div className="exercise-document-section-heading">
            <span className="exercise-document-section-number">2</span>
            <div>
              <span className="section-eyebrow">RÉPONSE</span>
              <h2>Rédiger la solution</h2>
            </div>
          </div>
          <p className="exercise-document-note">Présente les calculs, les unités et les justifications nécessaires. Évite de donner uniquement le résultat final.</p>
        </section>

        <div className="exercise-detail-actions exercise-document-actions">
          <Link to={`/exercices/${exercise.id}`} className="btn btn-primary">Ouvrir le runner <ArrowRight size={15} /></Link>
          <button type="button" className="btn btn-secondary" onClick={() => setShowCorrection((value) => !value)}><CheckCircle2 size={15} /> {showCorrection ? "Masquer" : "Voir"} la correction</button>
        </div>

        {showCorrection && <section className="exercise-correction-card exercise-document-correction">
          <span className="section-eyebrow">CORRECTION</span>
          <h2>Solution guidée</h2>
          <div className="exercise-correction-text"><LatexText>{exercise.correction}</LatexText></div>
          {exercise.hint && <div className="exercise-correction-hint"><Sparkles size={15} /><div><strong>Point de vigilance</strong><LatexText>{exercise.hint}</LatexText></div></div>}
        </section>}
      </article>
    </main>
  );
