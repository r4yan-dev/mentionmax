...
export default function Exercises() {
  ...
  const exercises = collection === "helios" ? heliosExercises : collection === "existing" ? existingExercises : [...heliosExercises, ...existingExercises];
  const chapters = useMemo(() => [...new Set(exercises.map((exercise) => exercise.target.chapter))], [exercises]);
  const selected = exerciseId ? [...heliosExercises, ...existingExercises].find((exercise) => exercise.id === exerciseId) : undefined;
  ...
  return <main className="exercises-page"><header className="exercises-header"><div><span className="section-eyebrow">BANQUE D’EXERCICES · 2BAC</span><h1>Travaille vraiment le cours.</h1><p>{tracks[trackId].label} · {filtered.length} exercice{filtered.length > 1 ? "s" : ""} affiché{filtered.length > 1 ? "s" : ""} pour {subjectLabels[subject]}.</p></div><div className="exercises-header__right"><PathSwitcher /><div className="exercises-header__stats"><span><BookOpen size={17} /> {exercises.length} affichés</span><span><FlaskConical size={17} /> {heliosExercises.length} Helios</span></div></div></header><section className="exercise-collection-switch"><button type="button" className={collection === "helios" ? "active" : ""} onClick={() => setCollection("helios")}><FlaskConical size={16} /> Mission Helios <span>{heliosExercises.length}</span></button><button type="button" className={collection === "all" ? "active" : ""} onClick={() => setCollection("all")}><BookOpen size={16} /> Tout <span>{heliosExercises.length + existingExercises.length}</span></button><button type="button" className={collection === "existing" ? "active" : ""} onClick={() => setCollection("existing")}><BrainCircuit size={16} /> Exercices existants <span>{existingExercises.length}</span></button></section><section className="exercise-filters card"><div className="exercise-filter-heading"><Filter size={16} /><strong>Filtrer</strong></div><label><span>Matière</span><select value={subject} onChange={(event) => { setSubject(event.target.value as SubjectId); setChapter("all"); }}>{allowedSubjects.map((id) => <option key={id} value={id}>{subjectLabels[id]}</option>)}</select></label><label><span>Chapitre</span><select value={chapter} onChange={(event) => setChapter(event.target.value)}><option value="all">Tous les chapitres</option>{chapters.map((value) => <option key={value}>{value}</option>)}</select></label><label><span>Difficulté</span><select value={difficulty} onChange={(event) => setDifficulty(event.target.value)}><option value="all">Toutes</option>{[1,2,3,4,5].map((value) => <option key={value} value={value}>{difficultyLabels[value]}</option>)}</select></label></section><section className="exercise-grid">{filtered.map((exercise) => exercise.tags.includes("MISSION_HELIOS") ? <HeliosExerciseWidget key={exercise.id} exercise={exercise} /> : <ExistingExerciseCard key={exercise.id} exercise={exercise} />)}</section></main>;
}
