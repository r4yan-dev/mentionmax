...
import type { Exercise, ExercisePart, ExerciseType } from "../types/content";
...
const typeLabels: Record<ExerciseType, string> = { mcq: "QCM", numeric: "Numérique", "short-answer": "Réponse courte", proof: "Démonstration", calculation: "Calcul", "document-analysis": "Analyse de document", "multi-step": "Sujet bac" };
...
function exerciseNumber(id: string) { const match = id.match(/(\d{1,4})$/); return match?.[1] ? Number(match[1]) : null; }

function PlainMath({ text }: { text: string }) {
  return <LatexText>{text}</LatexText>;
}

function StructuredPart({ part }: { part: ExercisePart }) {
  return (
    <section className="exercise-part">
      <div className="exercise-part__heading">
        <span className="exercise-part__number">Partie {part.number}</span>
        <div><span className="section-eyebrow">PARTIE {part.number}</span><h3>{part.title.replace(/^Partie [12] — /, "")}</h3></div>
      </div>
      {part.introduction && <p className="exercise-part__intro"><PlainMath text={part.introduction} /></p>}
      <ExerciseVisual visual={part.visual} />
      <div className="exercise-question-list">
        {part.questions.map((question) => (
          <section className="exercise-question" key={question.id}>
            <div className="exercise-question__header"><span>{question.number}.</span>{question.prompt && <PlainMath text={question.prompt} />}</div>
            <div className="exercise-subquestions">
              {question.subQuestions.map((sq) => (
                <div className="exercise-subquestion" key={sq.id}><strong>{sq.label}</strong><PlainMath text={sq.prompt} /></div>
              ))}
            </div>
          </section>
        ))}
...
function ExerciseCard({ exercise }: { exercise: DisplayExercise }) {
  const partCount = exercise.parts?.length ?? 0;
  const questionCount = exercise.parts?.reduce((sum, part) => sum + part.questions.length, 0) ?? 0;
  return <article className="exercise-library-card"><div className="exercise-library-card__top"><span className="exercise-pill">{exerciseTypeLabel(exercise.type)}</span><span className={`exercise-difficulty difficulty-${exercise.difficulty}`}>{difficultyLabels[exercise.difficulty]}</span></div><div className="exercise-library-card__icon"><Target size={20} /></div><div className="exercise-library-card__body"><span className="exercise-library-card__chapter">{exercise.target.chapter} · {exercise.target.topic}</span><h3>{exercise.title}</h3><p><PlainMath text={exercise.statement} /></p>{partCount > 0 && <div className="exercise-goal"><Target size={14} /><span><strong>Format bac</strong>{partCount} parties · {questionCount} questions structurées</span></div>}</div><div className="exercise-library-card__footer"><span><Clock3 size={14} /> {exercise.estimatedMinutes} min · +{exercise.xpValue} XP</span><Link to={`/exercices/${exercise.id}`} className="exercise-open-link">Ouvrir <ArrowRight size={15} /></Link></div></article>;
}

function Correction({ exercise }: { exercise: DisplayExercise }) {
  return <section className="exercise-correction-card exercise-document-correction"><span className="section-eyebrow">CORRECTION</span><h2>Corrigé du sujet</h2>{exercise.parts ? exercise.parts.map((part) => <div key={`${part.id}-corr`} className="exercise-correction-part"><h3>Partie {part.number} — {part.title.replace(/^Partie [12] — /, "")}</h3>{part.questions.map((question) => <div key={`${question.id}-corr`} className="exercise-correction-question"><strong>{question.number}.</strong>{question.subQuestions.map((sq) => <p key={`${sq.id}-corr`}><b>{sq.label}</b> {sq.correction}</p>)}</div>)}</div>) : <div className="exercise-correction-text"><PlainMath text={exercise.correction} /></div>}</section>;
}

function ExerciseDetail({ exercise }: { exercise: DisplayExercise }) {
  const [showCorrection, setShowCorrection] = useState(false);
  const number = exerciseNumber(exercise.id);
  return (
    <main className="exercises-page exercises-detail-page">
      ...
      <article className="exercise-detail-card exercise-detail-card--document">
        <header className="exercise-document-header">
          <div className="exercise-document-header__brand"><span className="exercise-document-kicker">MENTIONMAX · 2BAC</span><span className="exercise-document-mode">SUJET D’ENTRAÎNEMENT</span></div>
          <div className="exercise-document-header__rule" />
          <div className="exercise-document-meta"><span>{subjectLabels[exercise.target.subjectId]}</span><span>{tracks[exercise.target.trackIds[0]].label}</span>{number !== null && <span>Exercice {number}</span>}</div>
        </header>
        <section className="exercise-document-title">
          <div className="exercise-document-title__line"><span className="exercise-document-section">SUJET</span><div className="exercise-document-badges"><span className="exercise-pill">{exerciseTypeLabel(exercise.type)}</span><span className={`exercise-difficulty difficulty-${exercise.difficulty}`}>{difficultyLabels[exercise.difficulty]}</span></div></div>
          <span className="exercise-library-card__chapter">{exercise.target.chapter} · {exercise.target.topic}</span><h1>{exercise.title}</h1>
          {exercise.examTip && <div className="exercise-goal exercise-goal--detail"><Target size={17} /><div><strong>Compétence évaluée</strong><span>{exercise.examTip}</span></div></div>}
        </section>
        <section className="exercise-document-section-block">
          <div className="exercise-document-section-heading"><span className="exercise-document-section-number">1</span><div><span className="section-eyebrow">ÉNONCÉ</span><h2>Problème</h2></div></div>
          {exercise.parts ? <div className="exercise-parts">{exercise.parts.map((part) => <StructuredPart key={part.id} part={part} />)}</div> : <div className="exercise-document-copy"><PlainMath text={exercise.statement} /></div>}
        </section>
        <div className="exercise-detail-actions exercise-document-actions"><button type="button" className="btn btn-secondary" onClick={() => setShowCorrection((value) => !value)}><CheckCircle2 size={15} /> {showCorrection ? "Masquer" : "Voir"} le corrigé</button></div>
...
export default function Exercises() {
  ...
  useEffect(() => { const requested = searchParams.get("subject") as SubjectId | null; setSubject(requested && allowedSubjects.includes(requested) ? requested : allowedSubjects[0]); setChapter("all"); setDifficulty("all"); setVisibleCount(12); }, [trackId, searchParams, allowedSubjects.join(",")]);
