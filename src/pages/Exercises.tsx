...
function ExerciseCard({ exercise }: { exercise: Exercise }) {
  const helios = exercise as Exercise & Partial<HeliosExerciseMeta>;
  return <article className="exercise-library-card"><div className="exercise-library-card__top"><span className="exercise-pill">{typeLabels[exercise.type]}</span><span className={`exercise-difficulty difficulty-${exercise.difficulty}`}>{difficultyLabels[exercise.difficulty]}</span></div><div className="exercise-library-card__icon"><Target size={20} /></div><div className="exercise-library-card__body"><span className="exercise-library-card__chapter">{exercise.target.chapter}{helios.missionDay ? ` · Jour ${String(helios.missionDay).padStart(2,"0")}` : ""}</span><h3>{exercise.title}</h3><p><LatexText>{helios.context || exercise.statement}</LatexText></p></div><div className="exercise-library-card__footer"><span><Clock3 size={14} /> {exercise.estimatedMinutes} min · +{exercise.xpValue} XP</span><Link to={`/exercices/${exercise.id}`} className="exercise-open-link">Ouvrir <ArrowRight size={15} /></Link></div></article>;
}
  ...
  return <main className="exercises-page exercises-detail-page">
    ...
    <div className="exercise-detail-layout">
      <article className="exercise-detail-card">
        <div className="exercise-library-card__top"><span className="exercise-pill">{typeLabels[exercise.type]}</span><span className={`exercise-difficulty difficulty-${exercise.difficulty}`}>{difficultyLabels[exercise.difficulty]}</span></div>
        <span className="exercise-library-card__chapter">{exercise.target.chapter} · {exercise.target.topic}</span>
        <h1>{exercise.title}</h1>
        {helios.missionDay ? <div className="card" style={{ margin: "16px 0", padding: "16px" }}><span className="section-eyebrow">MISSION HELIOS · JOUR {String(helios.missionDay).padStart(2,"0")}</span><strong style={{ display: "block", marginTop: 6 }}>{helios.missionObjective}</strong></div> : null}
        {helios.context ? <section className="card" style={{ margin: "16px 0", padding: "18px" }}><span className="section-eyebrow">CONTEXTE DE MISSION</span><p style={{ margin: "8px 0 0", lineHeight: 1.7 }}><LatexText>{helios.context}</LatexText></p></section> : null}
        {helios.parts?.length ? <section className="card" style={{ margin: "16px 0", padding: "18px" }}><span className="section-eyebrow">TRAVAIL À EFFECTUER</span><ol style={{ margin: "12px 0 0", paddingLeft: 24, lineHeight: 1.8 }}>{helios.parts.map((part, i) => <li key={`${exercise.id}-part-${i}`}><LatexText>{part}</LatexText></li>)}</ol></section> : <p className="exercise-detail-statement"><LatexText>{exercise.statement}</LatexText></p>}
        {helios.animation ? <div style={{ margin: "16px 0", padding: "12px 14px", borderRadius: 12, background: "var(--seafoam, #DDF5F1)" }}><strong>Animation :</strong> {helios.animation}</div> : null}
        {exercise.type === "mcq" && exercise.options ? <div className="exercise-options">{exercise.options.map((option) => <label key={option.id} className="exercise-option"><input type="radio" name="answer" value={option.id} /><span><LatexText>{option.label}</LatexText></span></label>)}</div> : <textarea value={answer} onChange={(event) => setAnswer(event.target.value)} className="exercise-answer-box" placeholder="Écris ton raisonnement ou ta réponse ici..." rows={8} />}
        <div className="exercise-detail-actions"><button type="button" className="btn btn-primary" onClick={() => setShowCorrection(true)}>Voir la correction</button><span><Clock3 size={14} /> {exercise.estimatedMinutes} min · +{exercise.xpValue} XP</span></div>
      </article>
      <aside className="exercise-detail-side"><div className="exercise-side-card"><span className="section-eyebrow">Objectif</span><h2>Travaille ce chapitre</h2><p>{exercise.examTip || "Consolide la méthode avant de passer à un exercice plus difficile."}</p></div><div className="exercise-side-card exercise-side-card--mint"><Sparkles size={18} /><div><strong>Personnalisé</strong><p>Le mode personnalisé sera branché sur tes erreurs une fois l'API IA disponible.</p></div></div></aside>
    </div>
    {showCorrection && <section className="exercise-correction-card"><div className="exercise-correction-card__icon"><CheckCircle2 size={20} /></div><div><span className="section-eyebrow">CORRECTION</span><h2>Solution guidée</h2><p><LatexText>{exercise.correction}</LatexText></p>{exercise.hint && <p className="exercise-hint"><strong>Réflexe :</strong> <LatexText>{exercise.hint}</LatexText></p>}</div></section>}
  </main>;
}
...
export default function Exercises() {
  ...
  useEffect(() => { const nextRequested = searchParams.get("subject") as SubjectId | null; if (nextRequested && allowedSubjects.includes(nextRequested)) setSubject(nextRequested); else if (!allowedSubjects.includes(subject)) setSubject(allowedSubjects[0]); setChapter("all"); }, [trackId, searchParams, allowedSubjects.join(",")]);
