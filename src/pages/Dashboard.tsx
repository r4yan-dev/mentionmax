...
function calculateStreak(activityDates: string[]) {
  const days = new Set(
    activityDates.map((value) => startOfLocalDay(new Date(value)).getTime()),
  );
  let streak = 0;
  ...
  if (streak === 0) {
    ...
    while (days.has(yesterday.getTime())) {
      streak += 1;
      yesterday.setDate(yesterday.getDate() - 1);
    }
  }
...
export default function Dashboard() {
  ...
  const pathLabel = schoolPreferences?.track === "SM"
    ? `Sciences Mathématiques ${schoolPreferences.section ?? ""}`
    : "Sciences Physiques";
    ...
    async function loadActivity() {
      ...
      try {
        ...
        const focusSeconds = focusRows.reduce(
          (sum, row) => sum + Number(row.actual_seconds ?? 0),
          0,
        );
        ...
        for (const row of focusRows) {
          ...
          weekFocusByDay[dayIndex] += Number(row.actual_seconds ?? 0);
        }
        ...
        setActivity({
          completedExercises,
          totalExercises: totalResult.count ?? 0,
          totalFocusSeconds: focusSeconds,
          weekFocusSeconds: weekFocusByDay.reduce((sum, value) => sum + value, 0),
          streakDays: calculateStreak(activityDates),
          weekFocusByDay,
        });
      } catch (error) {
        console.error("Failed to load dashboard activity:", error);
        if (mounted) setActivity(emptyActivity);
      } finally {
  ...
  const exerciseProgress = activity.totalExercises > 0
    ? Math.round((activity.completedExercises / activity.totalExercises) * 100)
    : 0;
  ...
  const realMastery = useMemo(() => {
    if (activity.totalExercises === 0) return 0;
    return exerciseProgress;
  }, [activity.totalExercises, exerciseProgress]);
