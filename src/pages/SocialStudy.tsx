  ...
  return (
    <main className="social-page app-page">
      ...
      <section className="social-room-grid">
        ...
        <div className="social-leader-card card"><div className="social-card-heading"><div><span className="section-eyebrow">Cette semaine</span><h2>Qui garde le rythme ?</h2></div><Link to={`/focus/groups/${selectedGroup.id}`}><ArrowRight size={16}/></Link></div><div className="social-leader-list">{leaders.length ? leaders.map((entry, index) => <div className="social-leader-row" key={entry.userId}><strong className="social-leader-rank">#{index + 1}</strong><span className="social-avatar">{entry.avatarUrl ? <img src={entry.avatarUrl} alt=""/> : initials(entry.displayName)}</span><div><strong>{entry.displayName}</strong><span>{formatMinutes(entry.totalSeconds)} de focus</span></div><b>{index === 0 ? "🏆" : index === 1 ? "🥈" : index === 2 ? "🥉" : ""}</b></div>) : <div className="social-muted">Aucune session cette semaine. Quel dommage pour le classement, mais quelle occasion de le remplir.</div>}</div></div>
      </section>
